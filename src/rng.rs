...
impl Rng {
    ...
    fn potential_pokes(&self) -> Vec<Poke> {
        ...
        //                 let [rand_1, rand_2] = self.poke_rands(
        //                     a_div_1_offset,
        //                     s_div_1_offset,
        //                     a_div_2_offset,
        //                     s_div_2_offset,
        ...
        //                 let poke = Poke::new(rand_1[1], rand_2[1]);
        //                 if poke.is_shiny {
        //                     println!(
        //                         "{} {} {} {} {:?}",
        //                         a_div_1_offset,
    ...
    /// This doesn't work, but it's roughly the right idea
    pub fn generate_celebi(&self) -> Poke {
        let mut poke_rng = self.clone();
        ...
        // Advance 596 has a div -0xc
        poke_rng.next_with_div_offset(Offset::Minus(0xc));

        // It returns back on advance 957
        for _ in 597..600 {
            poke_rng.next();
        }

        poke_rng.add_div.increment_value(0xba);
        poke_rng.sub_div.increment_value(0xba);
        // This might not be quite right
        // This seems right on the pattern of 0x10, but not 0x4000
        poke_rng.add_div.increment_index(10);
        poke_rng.sub_div.increment_index(10);
        poke_rng.next();
        // println!("advances: {}, {:x?}", advances + 1, poke_rng);

        for _ in 600..612 {
            poke_rng.next();
        }

        poke_rng.add_div.increment_value(0x7c);
        poke_rng.sub_div.increment_value(0x7c);
        poke_rng.add_div.increment_index(14);
        poke_rng.sub_div.increment_index(14);
        poke_rng.next();

        poke_rng.add_div.increment_value(0xf3);
        poke_rng.sub_div.increment_value(0xf3);
        poke_rng.add_div.increment_index(8); // 5, 8, 11 - leaning towards 8 and 11
        poke_rng.sub_div.increment_index(8); // 5, 8, 11 - leaning towards 8 and 11
        let [_, atkdef] = poke_rng.next();

        poke_rng.add_div.increment_value(0x2);
        poke_rng.sub_div.increment_value(0x2);
        let [_, spespc] = advance_rng(
            poke_rng.r_add,
            poke_rng.r_sub,
            poke_rng.add_div.value(),
            poke_rng.sub_div.value(),
        );

        Poke::new(atkdef, spespc)
    }
}
